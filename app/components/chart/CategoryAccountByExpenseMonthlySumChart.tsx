...
function CustomAngleAxisTick({ payload, x, y, textAnchor, stroke, radius, chartData, highlightTopX, highlightTopCount }: any) {
  ...
  if (accountName) {
    ...
    if (isAccountHighestSum) {
      ...
      ellipsisName = `${ellipsisName} ($${highestSumAccount.expensesSum})`;
      ellipsisName = ellipsisName.length > 22 ? `${ellipsisName.substring(0, 22)}...` : ellipsisName;
      return (
        <g
  ...
  return (
    ...
    >
      <text
        ...
        textAnchor={ textAnchor }
      >
        <tspan x={ x + 8 } y={ y } fill={ '#ccc' } fontSize="11px">
          ${ payload.value }
        </tspan>
      </text>
    </g>
